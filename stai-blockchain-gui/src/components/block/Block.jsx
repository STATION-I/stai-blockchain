...
export default function Block() {
  ...
  const poolReward = mojo_to_stai(calculatePoolReward(blockRecord.height));
  const baseFarmerReward = mojo_to_stai(
    calculateBaseFarmerReward(blockRecord.height),
  );

  const staiFees = blockRecord.fees
    ? mojo_to_stai(BigInt(blockRecord.fees))
    : '';
  ...
  const rows = [
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://www.staiexplorer.org/blockchain/puzzlehash/${blockRecord.farmer_puzzle_hash}`}
        >
          {currencyCode
            ? toBech32m(
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://www.staiexplorer.org/blockchain/puzzlehash/${blockRecord.pool_puzzle_hash}`}
        >
          {currencyCode
            ? toBech32m(
