...
import { /* mojo_to_stai_string, */ stai_to_mojo } from '../../../util/stai';
  ...
  return (
    <Grid item xs={12}>
      <Box display="flex">
        ...
        <Box>
          <Typography variant="subtitle1">
            {mojo_to_stai_string(props.balance)} {currencyCode}
          </Typography>
        </Box>
      </Box>
    </Grid>
  ...
  return (
    <Card title={<Trans>Balance</Trans>}>
      <BalanceCardSubSection
        ...
        tooltip={
          <Trans>
            This is the total amount of stai in the blockchain at the current
      ...
      <BalanceCardSubSection
        ...
        tooltip={
          <Trans>
            This is the amount of Stai that you can currently use to make
            transactions. It does not include pending farming rewards, pending
            incoming transactions, and Stai that you have just spent but is not
  ...
  function handleSubmit(data: SendTransactionData) {
    ...
    if (address.includes('colour')) {
      openDialog(
        <AlertDialog>
          <Trans>
            Error: Cannot send stai to coloured address. Please enter a stai
            address.
          </Trans>
        </AlertDialog>,
      );
    ...
    if (address.slice(0, 12) === 'stai_addr://') {
    ...
    const amountValue = Number.parseFloat(stai_to_mojo(amount));
    const feeValue = Number.parseFloat(stai_to_mojo(fee));
