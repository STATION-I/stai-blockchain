...
import { AlertDialog } from '@stai/core';
import {
  Typography,
  Button,
  Box,
...
import { stai_to_mojo } from '../../../util/stai';
...
export const CreateRLAdminWallet = () => {
  ...
  function create() {
    ...
    if (
      staiper_input.value === '' ||
      Number(staiper_input.value) === 0 ||
      !Number(staiper_input.value) ||
      isNaN(Number(staiper_input.value))
    ) {
      dispatch(
        openDialog(
          <AlertDialog>
    ...
    const staiper = stai_to_mojo(staiper_input.value);
    const staiper_value = Number.parseInt(Number(staiper));
    ...
    // var fee = stai_to_mojo(fee_input.value);
