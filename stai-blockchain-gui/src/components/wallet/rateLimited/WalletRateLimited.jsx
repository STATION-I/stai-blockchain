...
import { AlertDialog, Card, Flex } from '@stai/core';
import {
  send_transaction,
...
const IncompleteCard = (props) => {
  ...
  function submit() {
    ...
    const staiper_input = ip_parsed.limit;
    ...
    const staiper_value = Number.parseInt(Number(staiper_input));
        ...
        <Grid item xs={12}>
          <Box display="flex">
            ...
            <Box flexGrow={1}>
              <Typography variant="subtitle1">
                <Trans>
                  Spending Limit (stai per interval):{' '}
                  {mojo_to_stai_string(limit)}
                </Trans>
              </Typography>
            </Box>
          </Box>
...
const SendCard = (props) => {
  ...
  function send() {
    ...
    const amount = stai_to_mojo(amount_input.value);
    const fee = stai_to_mojo(fee_input.value);
