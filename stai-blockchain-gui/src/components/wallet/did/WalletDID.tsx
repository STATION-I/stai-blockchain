...
import { AlertDialog, Card, Flex, Loading, Dropzone } from '@stai/core';
import {
...
import { mojo_to_stai_string } from '../../../util/stai';
  ...
  return (
    <Grid item xs={12}>
      <Box display="flex">
        ...
        <Box>
          <Typography variant="subtitle1">
            {mojo_to_stai_string(props.balance)} TSTAI
          </Typography>
        </Box>
      </Box>
    </Grid>
...
const CreateAttest = (props) => {
  ...
  function createAttestPacket() {
    ...
    if (address.substring(0, 12) === 'stai_addr://') {
...
const CashoutCard = (props) => {
  ...
  function cashout() {
    ...
    if (puzzlehash.slice(0, 12) === 'stai_addr://') {
