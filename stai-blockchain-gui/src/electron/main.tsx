...
import staiEnvironment from '../util/staiEnvironment';
import staiConfig from '../util/config';
...
if (!handleSquirrelEvent()) {
  ...
  const ensureCorrectEnvironment = () => {
    ...
    if (!staiEnvironment.guessPackaged() && !('VIRTUAL_ENV' in process.env)) {
  ...
  if (ensureSingleInstance() && ensureCorrectEnvironment()) {
    ...
    const createWindow = async () => {
      ...
      if (!staiConfig.manageDaemonLifetime()) {
      ...
      mainWindow.on('close', (e) => {
        ...
        if (decidedToClose || !staiConfig.manageDaemonLifetime()) {
          return;
        }
        e.preventDefault();
        if (!isClosing) {
    ...
    const appReady = async () => {
      ...
      // if the daemon isn't local we aren't going to try to start/stop it
      if (staiConfig.manageDaemonLifetime()) {
        staiEnvironment.startStaiDaemon();
      }
    };
  ...
  const getMenuTemplate = () => {
    const template = [
      ...
      },
      {
        ...
        submenu: [
          {
            label: i18n._(/* i18n */ { id: 'Find more about our project' }),
            click: () => {
              openExternal(
                'https://www.station-i.de/',
              );
            },
          },
          {
            label: i18n._(/* i18n */ { id: 'Stai Blockchain Wiki' }),
            click: () => {
              openExternal(
                'https://github.com/STATION-I/stai-blockchain/wiki',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/STATION-I/stai-blockchain/releases',
              );
            },
          },
          {
            label: i18n._(/* i18n */ { id: 'Report an issue to our team' }),
            click: () => {
              openExternal(
                'https://discord.gg/qpR4Mf6S4t',
              );
            },
          },
          {
            label: i18n._(/* i18n */ { id: 'Download latest database file' }),
            click: () => {
              openExternal(
          ...
          {
            label: i18n._(/* i18n */ { id: 'Join our Facebook group' }),
            click: () => {
              openExternal(
                'https://www.facebook.com/groups/staiglobalsupportgroup/',
              );
            },
          },
          {
    ...
    if (process.platform === 'darwin') {
      // Stai Blockchain menu (Mac)
      template.unshift({
        label: i18n._(/* i18n */ { id: 'Stai' }),
        submenu: [
          {
            label: i18n._(/* i18n */ { id: 'About Stai Blockchain' }),
            click: () => {
              openAbout();
            },
          },
