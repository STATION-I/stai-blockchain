...
import { AlertDialog, Card, Flex, staiToMojo, mojoToStaiLocaleString } from '@stai/core';
import {
  send_transaction,
  ...
  return (
    <Grid item xs={12}>
      <Box display="flex">
        ...
        <Box>
          <Typography variant="subtitle1">
            {mojoToStaiLocaleString(props.balance)} {currencyCode}
          </Typography>
        </Box>
      </Box>
    </Grid>
...
const SendCard = (props) => {
  ...
  function send() {
    ...
    const amount = staiToMojo(amount_input.value);
    const fee = staiToMojo(fee_input.value);
