...
import { useGetWalletBalanceQuery } from '@stai/api-react';
import { TransactionType } from '@stai/api';
import type { Transaction } from '@stai/api';
import { mojoToStai, blockHeightToTimestamp } from '@stai/core';
  ...
  */
  ...
  const points = [
    {
      ...
      y: BigNumber.max(0, mojoToStai(start)).toNumber(), // max 21,000,000 safe to number
      tooltip: mojoToStai(balance).toString(), // bignumber is not supported by react
    },
  ];
  ...
  data.forEach((item) => {
    ...
    points.push({
      x: timestamp,
      y: BigNumber.max(0, mojoToStai(start)).toNumber(), // max 21,000,000 safe to number
      tooltip: mojoToStai(start).toString, // bignumber is not supported by react
    });
  });
