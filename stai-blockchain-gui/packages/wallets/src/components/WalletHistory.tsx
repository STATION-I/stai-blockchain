...
import { Card, CardKeyValue, CopyToClipboard, Flex, Loading, StateColor, TableControlled, toBech32m, useCurrencyCode, mojoToStaiLocaleString, mojoToCATLocaleString } from '@stai/core';
import { useGetOfferRecordMutation, useGetSyncStatusQuery } from '@stai/api-react';
import styled from 'styled-components';
import type { Row } from '@stai/core';
import { WalletType, TransactionType } from '@stai/api';
...
const getCols = (type: WalletType, isSyncing, getOfferRecord, navigate) => [
        ...
        <>
          ...
          <strong>
            {type === WalletType.CAT
              ...
              : mojoToStaiLocaleString(row.amount)}
          </strong>
          &nbsp;
          {metadata.unit}
        </>
  ...
  },
  {
    field: (row: Row, metadata) => (
      <>
        <strong>{mojoToStaiLocaleString(row.feeAmount)}</strong>
        &nbsp;
        {metadata.feeUnit}
      </>
    ),
