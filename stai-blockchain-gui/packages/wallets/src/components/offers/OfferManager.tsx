...
import { OfferTradeRecord } from '@stai/api';
import fs from 'fs';
...
import { Trade as TradeIcon } from '@stai/icons';
import { useCancelOfferMutation, useGetOfferDataMutation, useGetWalletsQuery } from '@stai/api-react';
...
function ConfirmOfferCancellation(props: ConfirmOfferCancellationProps) {
  ...
  async function handleConfirm() {
    const { fee: staiFee } = methods.getValues();
    var fee = 0;

    if (cancelWithTransaction) {
      fee = Number.parseFloat(staiToMojo(staiFee));
    }
      ...
      <Grid container>
        <Grid xs={12} md={6} lg={5} item>
          <CardHero>
            ...
            <Typography variant="body1">
              <Trans>
                Create an offer to exchange STAI or other tokens. View an offer to inspect and accept an offer made by another party.
              </Trans>
            </Typography>
