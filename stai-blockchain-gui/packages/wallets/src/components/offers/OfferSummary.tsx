...
import { type OfferSummaryRecord  } from '@stai/api';
import {
  Flex,
  StateColor,
  mojoToStai,
...
export default function OfferSummary(props: Props) {
  ...
  const makerAmount = makerEntries[0][0].toLowerCase() === 'stai' ? Number(mojoToStai(makerEntries[0][1])) : Number(mojoToCAT(makerEntries[0][1]));
  const takerAmount = takerEntries[0][0].toLowerCase() === 'stai' ? Number(mojoToStai(takerEntries[0][1])) : Number(mojoToCAT(takerEntries[0][1]));
