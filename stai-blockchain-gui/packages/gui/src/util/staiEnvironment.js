...
const getStaiVersion = () => {
  let version = null;
  const exePath = getExecutablePath('stai');
  // first see if we can get a stai exe in a standard location relative to where we are
  try {
    ...
    // that didn't work, let's try as if we're in the venv or stai is on the path
    try {
      version = child_process
