...
export default function OfferSummary(props: Props) {
  ...
  const makerAmount =
    makerEntries.length > 0
      ? ['stai', 'tstai'].includes(makerEntries[0][0].toLowerCase())
        ? mojoToStai(makerEntries[0][1])
        : mojoToCAT(makerEntries[0][1])
      : undefined;
  const takerAmount =
    takerEntries.length > 0
      ? ['stai', 'tstai'].includes(takerEntries[0][0].toLowerCase())
        ? mojoToStai(takerEntries[0][1])
