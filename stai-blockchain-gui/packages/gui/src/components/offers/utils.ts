...
export function summaryStringsForNFTOffer(
  ...
  // TODO: Remove 1:1 NFT <--> STAI assumption
...
export function formatAmountForWalletType(
  ...
  if (walletType === WalletType.STANDARD_WALLET) {
    return mojoToStaiLocaleString(amount, locale);
...
export function offerAssetTypeForAssetId(
  ...
  if (['stai', 'tstai'].includes(assetId)) {
...
export function getNFTPriceWithoutRoyalties(
  ...
  // NFTs can only be exchanged for STAI currently
  const amountInMojos = offerAssetAmountForAssetId('stai', summary);
  if (amountInMojos === undefined) {
    return undefined;
  }
  return mojoToStai(amountInMojos).toNumber();
}
