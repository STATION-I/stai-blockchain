...
import type { NFTInfo, Wallet } from '@stai/api';
import {
...
/*        An NFT offer consists of a single NFT being offered for STAI         */
/* ========================================================================== */
...
function NFTOfferConditionalsPanel(props: NFTOfferConditionalsPanelProps) {
  ...
  const amount = methods.watch('staiAmount');
  ...
  const offerElem =
    tab === NFTOfferExchangeType.NFTForSTAI ? nftElem : amountElem;
  const takerElem =
    tab === NFTOfferExchangeType.NFTForSTAI ? amountElem : nftElem;
  ...
  function handleAmountChange(amount: string) {
    methods.setValue('staiAmount', amount);
  }
    ...
    >
      ...
      >
        <Tab
          ...
          label={<Trans>NFT for STAI</Trans>}
          disabled={disabled}
        />
        <Tab
          value={NFTOfferExchangeType.XCHForNFT}
          label={<Trans>STAI for NFT</Trans>}
          disabled={disabled}
        />
      </Tabs>
      <Grid container>
              ...
              <>
                <Flex flexDirection="column" gap={0.5}>
                  <Typography variant="body1" color="textSecondary">
                    {tab === NFTOfferExchangeType.NFTForSTAI ? (
...
/* ========================================================================== */
/*                              NFT Offer Editor                              */
/*             Currently only supports a single NFT <--> STAI offer            */
/* ========================================================================== */
...
function buildOfferRequest(
  ...
  const baseMojoAmount: BigNumber = staiToMojo(staiAmount);
  const mojoAmount =
    exchangeType === NFTOfferExchangeType.NFTForXCH
      ? baseMojoAmount
      : baseMojoAmount.negated();
  const feeMojoAmount = staiToMojo(fee);
  const nftAmount = exchangeType === NFTOfferExchangeType.NFTForSTAI ? -1 : 1;
  const staiWalletId = 1;
...
export default function NFTOfferEditor(props: NFTOfferEditorProps) {
  ...
  function validateFormData(
    ...
    const { exchangeType, nftId, staiAmount, fee } = unvalidatedFormData;
    ...
    } else if (!staiAmount || staiAmount === '0') {
  ...
  async function handleSubmit(unvalidatedFormData: NFTOfferEditorFormData) {
    ...
    const { exchangeType, launcherId, staiAmount, fee } = formData;
