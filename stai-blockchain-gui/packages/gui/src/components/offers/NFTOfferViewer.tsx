...
import { OfferSummaryRecord, OfferTradeRecord } from '@stai/api';
import {
  Back,
  Button,
  ButtonLoading,
...
function NFTOfferSummaryRow(props: NFTOfferSummaryRowProps) {
  ...
  const assetIdsToTypes: { [key: string]: OfferAsset | undefined }[] =
    useMemo(() => {
      return Object.keys(summaryData).map((key) => {
        ...
        if (['stai', 'tstai'].includes(key.toLowerCase())) {
          assetType = OfferAsset.STAI;
  ...
  const rows: (React.ReactElement | null)[] = assetIdsToTypes.map((entry) => {
    ...
    switch (assetType) {
      case undefined:
        return null;
      case OfferAsset.STAI: // fall-through
...
function NFTOfferDetails(props: NFTOfferDetailsProps) {
  ...
  const nftSaleInfo = useMemo(() => {
    ...
    const staiMakerFee = mojoToStai(makerFee);

    return {
      ...calculateNFTRoyalties(
        amount,
        parseFloat(staiMakerFee),
  ...
  const overrideNFTSellerAmount =
    exchangeType === NFTOfferExchangeType.XCHForNFT
      ? staiToMojo(nftSaleInfo?.nftSellerNetAmount ?? 0)
      : undefined;
              ...
              {nftSaleInfo && (
                <>
                  ...
                  <Flex flexDirection="column" gap={0.5}>
                    <Flex flexDirection="row" alignItems="center" gap={1}>
                      {exchangeType === NFTOfferExchangeType.NFTForSTAI ? (
