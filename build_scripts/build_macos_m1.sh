...
electron-packager . staicoin --asar.unpack="**/daemon/**" --platform=darwin \
...
if [ "$NOTARIZE" ]; then
  electron-osx-sign staicoin-darwin-arm64/staicoin.app --platform=darwin \
  --hardened-runtime=true --provisioning-profile=staicoinblockchain.provisionprofile \
...
mv staicoin-darwin-arm64 ../build_scripts/dist/
cd ../build_scripts || exit

DMG_NAME="staicoin-$STAICOIN_INSTALLER_VERSION-arm64.dmg"
echo "Create $DMG_NAME"
mkdir final_installer
electron-installer-dmg dist/staicoin-darwin-arm64/staicoin.app staicoin-$STAICOIN_INSTALLER_VERSION-arm64 \
...
if [ "$NOTARIZE" ]; then
  ...
  notarize-cli --file=$DMG_NAME --bundle-id net.staicoin.blockchain \
...
# xcrun altool --notarize-app -f staicoin-0.1.X.dmg --primary-bundle-id net.staicoin.blockchain -u username -p password
...
# xcrun stapler staple staicoin-0.1.X.dmg
#
# Validate DMG:
# xcrun stapler validate staicoin-0.1.X.dmg
