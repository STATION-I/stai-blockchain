...
from stai.protocols.protocol_timing import API_EXCEPTION_BAN_SECONDS, INVALID_PROTOCOL_BAN_SECONDS
...
from stai.util.network import is_in_network, is_localhost, select_port
...
class StaiServer:
    def __init__(
        ...
        self.all_connections: Dict[bytes32, WSStaiConnection] = {}
        self.tasks: Set[asyncio.Task] = set()

        self.connection_by_type: Dict[NodeType, Dict[bytes32, WSStaiConnection]] = {
            NodeType.FULL_NODE: {},
            NodeType.WALLET: {},
            NodeType.HARVESTER: {},
            NodeType.FARMER: {},
        ...
        self.stai_ca_crt_path, self.stai_ca_key_path = stai_ca_crt_key
    ...
    async def start_server(self, on_connect: Callable = None):
        ...
        if authenticate:
            ...
            ssl_context = ssl_context_for_server(
                self.stai_ca_crt_path, self.stai_ca_key_path, self.p2p_crt_path, self.p2p_key_path, log=self.log
            )
    ...
    async def connection_added(self, connection: WSStaiConnection, on_connect=None):
    ...
    async def start_client(
        ...
        if auth:
            ...
            ssl_context = ssl_context_for_client(
                self.stai_ca_crt_path, self.stai_ca_key_path, self.p2p_crt_path, self.p2p_key_path
            )
        session = None
        connection: Optional[WSStaiConnection] = None
        try:
    ...
    def connection_closed(self, connection: WSStaiConnection, ban_time: int):
    ...
    async def incoming_api_task(self) -> None:
        ...
        while True:
            ...
            async def api_call(full_message: Message, connection: WSStaiConnection, task_id):
    ...
    def get_outgoing_connections(self) -> List[WSStaiConnection]:
        result = []
        for _, connection in self.all_connections.items():
    ...
    def get_full_node_outgoing_connections(self) -> List[WSStaiConnection]:
        result = []
    ...
    def get_full_node_connections(self) -> List[WSStaiConnection]:
        return list(self.connection_by_type[NodeType.FULL_NODE].values())

    def get_connections(self, node_type: Optional[NodeType] = None) -> List[WSStaiConnection]:
        result = []
        for _, connection in self.all_connections.items():
    ...
    async def get_peer_info(self) -> Optional[PeerInfo]:
        ...
        try:
            ...
            async with ClientSession(timeout=timeout) as session:
                async with session.get("https://ip.stainetwork.org/") as resp:
                    if resp.status == 200:
    ...
    def is_trusted_peer(self, peer: WSStaiConnection, trusted_peers: Dict) -> bool:
