...
from stai.cmds.cmds_util import transaction_submitted_msg, transaction_status_msg
from stai.cmds.show import print_connections
...
from stai.server.start_wallet import SERVICE_NAME
...
from stai.wallet.nft_wallet.nft_info import NFT_HRP, NFTInfo
...
async def send(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    if typ == WalletType.STANDARD_WALLET:
        final_amount = uint64(int(amount * units["stai"]))
        final_min_coin_amount = uint128(int(min_coin_amount * units["stai"]))
    ...
    print(f"Do 'stai wallet get_transaction -f {fingerprint} -tx 0x{tx_id}' to get status")
...
async def make_offer(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    fee: int = int(Decimal(args["fee"]) * units["stai"])
        ...
        for item in [*offers, *requests]:
            ...
            if confirmation not in ["y", "yes"]:
                ...
                if offer is not None:
                    ...
                    print(f"Use stai wallet get_offers --id {trade_record.trade_id} -f {fingerprint} to view status")
                else:
...
async def print_trade_record(record, wallet_client: WalletRpcClient, summaries: bool = False) -> None:
    ...
    if summaries:
        ...
        print(f"Included Fees: {fees / units['stai']}")
    print("---------------")
...
async def take_offer(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    if nft_coin_id is not None and nft_royalty_percentage > 0:
        ...
        if nft_royalty_asset_id == "stai":
            nft_royalty_currency = "STAI"
        else:
            result = await cat_name_resolver(bytes32.fromhex(nft_royalty_asset_id))
            if result is not None:
                nft_royalty_currency = result[1]

        nft_royalty_divisor = units["stai"] if nft_royalty_asset_id == "stai" else units["cat"]
    ...
    print(f"Included Fees: {Decimal(offer.bundle.fees()) / units['stai']}")
...
async def get_wallet(wallet_client: WalletRpcClient, fingerprint: int = None) -> Optional[Tuple[WalletRpcClient, int]]:
    ...
    if len(fingerprints) == 0:
        print("No keys loaded. Run 'stai keys generate' or import a key")
        return None
