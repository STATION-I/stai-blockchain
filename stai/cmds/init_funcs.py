...
from stai.ssl.create_ssl import (
    ensure_ssl_dirs,
...
from stai.wallet.derive_keys import master_sk_to_pool_sk, master_sk_to_wallet_sk
...
def check_keys(new_root: Path, keychain: Optional[Keychain] = None) -> None:
    ...
    if "stai_target_address" not in config["farmer"]:
        print(
            f"Setting the stai destination for the farmer reward (1/8 plus fees, solo and pooling) to {all_targets[0]}"
        )
...
def create_all_ssl(root_path: Path):
    ...
    write_ssl_cert_and_key(stai_ca_crt_path, stai_ca_crt, stai_ca_key_path, stai_ca_key)
    ...
    stai_ca_crt, stai_ca_key = get_stai_ca_crt_key()
    generate_ssl_for_nodes(ssl_dir, stai_ca_crt, stai_ca_key, False, overwrite=False)
...
def init(create_certs: Optional[Path], root_path: Path, fix_ssl_permissions: bool = False, testnet: bool = False):
    if create_certs is not None:
        if root_path.exists():
            ...
            if (
                stai_init(root_path, fix_ssl_permissions=fix_ssl_permissions, testnet=testnet) == 0
        ...
        return stai_init(root_path, fix_ssl_permissions=fix_ssl_permissions, testnet=testnet)
...
def stai_minor_release_number():
    res = int(stai_version_number()[2])
...
def stai_init(
    ...
    """
    if os.environ.get("STAI_ROOT", None) is not None:
        print(
            f"warning, your STAI_ROOT is set to {os.environ['STAI_ROOT']}. "
            f"Please unset the environment variable and run stai init again\n"
