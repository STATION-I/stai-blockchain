...
from stai.consensus.cost_calculator import calculate_cost_of_program, NPCResult
...
class Wallet:
    ...
    async def create_spend_bundle_relative_stai(self, stai_amount: int, exclude: List[Coin]) -> SpendBundle:
        ...
        if stai_amount < 0:
            utxos = await self.select_coins(abs(stai_amount), exclude)
        else:
        ...
        for coin in utxos:
            ...
            if output_created is None:
                ...
                primaries = [{"puzzlehash": newpuzhash, "amount": stai_amount}]
