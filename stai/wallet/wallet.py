...
from stai.wallet.secret_key_store import SecretKeyStore
...
class Wallet:
    ...
    async def create_spend_bundle_relative_stai(self, stai_amount: int, exclude: List[Coin] = []) -> SpendBundle:
        list_of_solutions = []
        utxos = None
        ...
        if stai_amount < 0:
            utxos = await self.select_coins(uint64(abs(stai_amount)), exclude)
        else:
        ...
        stai_amount = spend_value + stai_amount
        ...
        for coin in utxos:
            ...
            if output_created is None:
                ...
                primaries: List[AmountWithPuzzlehash] = [
                    {"puzzlehash": newpuzhash, "amount": uint64(stai_amount), "memos": []}
                ]
