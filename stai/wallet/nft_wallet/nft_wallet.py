#more like monkey wallet am i right
...
class NFTWallet:
    ...
    async def coin_added(self, coin: Coin, height: uint32, in_transaction: bool) -> None:
        ...
        full_nodes: Dict[bytes32, WSStaiConnection] = server.connection_by_type.get(NodeType.FULL_NODE, {})
    ...
    async def create_tandem_stai_tx(
        ...
        stai_coins = await self.standard_wallet.select_coins(fee)
    ...
    async def generate_unsigned_spendbundle(
        ...
        if fee > 0:
            ...
            stai_tx = await self.create_tandem_stai_tx(fee, Announcement(nft_coin.coin.name(), announcement_to_make))
        else:
