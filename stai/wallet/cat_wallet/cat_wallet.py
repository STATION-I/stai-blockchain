...
from stai.wallet.cat_wallet.cat_info import CATInfo, LegacyCATInfo
...
from stai.wallet.puzzles.genesis_checkers import ALL_LIMITATIONS_PROGRAMS
...
class CATWallet:
    ...
    async def create_new_cat_wallet(
        ...
        stai_tx = dataclasses.replace(stai_tx, spend_bundle=spend_bundle)
                ...
                if need_stai_transaction:
                    if fee > regular_stai_to_claim:
                        stai_tx, _ = await self.create_tandem_stai_tx(
                            fee, uint64(regular_stai_to_claim), announcement_to_assert=announcement
                        )
                        ...
                        stai_tx, _ = await self.create_tandem_stai_tx(fee, uint64(regular_stai_to_claim))
                        innersol = self.standard_wallet.make_solution(
                            primaries=primaries,
                            coin_announcements={announcement.message},
