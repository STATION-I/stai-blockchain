...
from stai.cmds.init_funcs import check_keys, stai_init
...
from stai.server.server import ssl_context_for_root, ssl_context_for_server
...
class WebSocketServer:
    ...
    async def exit(self) -> Dict[str, Any]:
        ...
        log.info("stai daemon exiting in 5 seconds")
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service with STAI_ROOT: {os.environ['STAI_ROOT']}")
    ...
    if service_command == "stai_full_node_simulator":
        # Set the -D/--connect_to_daemon flag to signify that the child should connect
        # to the daemon to access the keychain
