...
def make_puzzle(amount: int) -> int:
    ...
    if error:
        ...
        for cvp in result_human:
            ...
            total_staicoin += int_from_bytes(cvp.vars[1])
            print(
                f"{ConditionOpcode(cvp.opcode).name}: {encode_puzzle_hash(cvp.vars[0], prefix)},"
                f" amount: {int_from_bytes(cvp.vars[1])}"
            )
    return total_staicoin


total_staicoin = 0
print("Pool address: ")
total_staicoin += make_puzzle(pool_amounts)
print("\nFarmer address: ")
total_staicoin += make_puzzle(farmer_amounts)
print("\nFarmer address: ")
total_staicoin += make_puzzle(officialwallets_amounts)

assert total_staicoin == calculate_base_officialwallets_reward(uint32(0)) + calculate_base_farmer_reward(uint32(0)) + calculate_pool_reward(uint32(0))
