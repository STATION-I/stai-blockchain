...
from staicoin.pools.pool_puzzles import SINGLETON_LAUNCHER_HASH
from staicoin.protocols import wallet_protocol
from staicoin.protocols.full_node_protocol import RequestProofOfWeight, RespondProofOfWeight
from staicoin.protocols.protocol_message_types import ProtocolMessageTypes
from staicoin.protocols.wallet_protocol import (
...
from staicoin.server.node_discovery import WalletPeers
...
from staicoin.util.errors import Err, ValidationError
from staicoin.util.ints import uint32, uint128
from staicoin.util.keychain import Keychain
from staicoin.util.lru_cache import LRUCache
from staicoin.util.merkle_set import MerkleSet, confirm_included_already_hashed, confirm_not_included_already_hashed
from staicoin.util.path import mkdir, path_from_root
from staicoin.wallet.block_record import HeaderBlockRecord
from staicoin.wallet.derivation_record import DerivationRecord
from staicoin.wallet.settings.settings_objects import BackupInitialized
from staicoin.wallet.transaction_record import TransactionRecord
from staicoin.wallet.util.backup_utils import open_backup_file
from staicoin.wallet.util.wallet_types import WalletType
from staicoin.wallet.wallet_action import WalletAction
from staicoin.wallet.wallet_blockchain import ReceiveBlockResult
...
class WalletNode:
    ...
    server: Optional[staicoinServer]
    log: logging.Logger
    ...
    def get_key_for_fingerprint(self, fingerprint: Optional[int]) -> Optional[PrivateKey]:
        ...
        if len(private_keys) == 0:
            self.log.warning("No keys present. Create keys with the UI, or with the 'staicoin keys' program.")
            return None
    ...
    async def complete_blocks(self, header_blocks: List[HeaderBlock], peer: WSstaicoinConnection):
    ...
    async def new_peak_wallet(self, peak: wallet_protocol.NewPeakWallet, peer: WSstaicoinConnection):
        if self.wallet_state_manager is None:
            return
    ...
    async def _sync(self) -> None:
        ...
        peers: List[WSstaicoinConnection] = self.server.get_full_node_connections()
        if len(peers) == 0:
    ...
    async def get_additions(
        self, peer: WSstaicoinConnection, block_i, additions: Optional[List[bytes32]], get_all_additions: bool = False
    ) -> Optional[List[Coin]]:
    ...
    async def get_removals(
        self, peer: WSstaicoinConnection, block_i, additions, removals, request_all_removals=False
    ) -> Optional[List[Coin]]:
        assert self.wallet_state_manager is not None
