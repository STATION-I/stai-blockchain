...
from staicoin.consensus.difficulty_adjustment import can_finish_sub_and_full_epoch
from staicoin.consensus.get_block_challenge import final_eos_is_already_included, get_block_challenge
from staicoin.consensus.make_sub_epoch_summary import make_sub_epoch_summary
...
from staicoin.types.blockchain_format.slots import ChallengeChainSubSlot, RewardChainSubSlot, SubSlotProofs
...
from staicoin.types.unfinished_header_block import UnfinishedHeaderBlock
...
def validate_unfinished_header_block(
    ...
    if genesis_block:
        ...
        if (
            ...
            return None, ValidationError(Err.INVALID_PREFARM)
        if (
            header_block.foliage.foliage_block_data.officialwallets_reward_puzzle_hash
            != constants.GENESIS_PRE_FARM_OFFICIALWALLETS_PUZZLE_HASH
        ):
            return None, ValidationError(Err.INVALID_OFFICIALWALLETS)
    else:
