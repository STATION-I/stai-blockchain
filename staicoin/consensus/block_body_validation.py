...
from staicoin.consensus.block_root_validation import validate_block_merkle_roots
from staicoin.full_node.mempool_check_conditions import mempool_check_conditions_dict
from staicoin.consensus.blockchain_interface import BlockchainInterface
from staicoin.consensus.coinbase import create_officialwallets_coin, create_farmer_coin, create_pool_coin
...
from staicoin.util import cached_bls
...
from staicoin.util.generator_tools import (
    additions_for_npc,
...
async def validate_block_body(
    ...
    if height > 0:
        ...
        officialwallets_coin = create_officialwallets_coin(
            prev_transaction_block_height,
            constants.GENESIS_PRE_FARM_OFFICIALWALLETS_PUZZLE_HASH,
            calculate_base_officialwallets_reward(prev_transaction_block.height),
            constants.GENESIS_CHALLENGE,
        )
        # Adds the previous block
        expected_reward_coins.add(pool_coin)
        expected_reward_coins.add(farmer_coin)
        expected_reward_coins.add(officialwallets_coin)
        ...
        if prev_transaction_block.height > 0:
            ...
            while not curr_b.is_transaction_block:
                ...
                expected_reward_coins.add(
                        create_officialwallets_coin(
                        curr_b.height,
                        constants.GENESIS_PRE_FARM_OFFICIALWALLETS_PUZZLE_HASH,
                        calculate_base_officialwallets_reward(curr_b.height),
                        constants.GENESIS_CHALLENGE,
                    )
                )
    ...
    # 18. Check that the fee amount + farmer reward < maximum coin amount
    if fees + calculate_base_officialwallets_reward(height) > constants.MAX_COIN_AMOUNT:
        return Err.COIN_AMOUNT_EXCEEDS_MAXIMUM, None
