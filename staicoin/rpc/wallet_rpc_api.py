...
from staicoin.cmds.init_funcs import check_keys
from staicoin.consensus.block_rewards import calculate_base_farmer_reward, calculate_base_officialwallets_reward
from staicoin.pools.pool_wallet import PoolWallet
from staicoin.pools.pool_wallet_info import create_pool_state, FARMING_TO_POOL, PoolWalletInfo, PoolState
...
from staicoin.util.keychain import bytes_to_mnemonic, generate_mnemonic
from staicoin.util.path import path_from_root
from staicoin.util.ws_message import WsRpcMessage, create_payload_dict
...
from staicoin.wallet.derive_keys import master_sk_to_farmer_sk, master_sk_to_pool_sk, master_sk_to_wallet_sk
...
from staicoin.wallet.util.backup_utils import download_backup, get_backup_info, upload_backup
from staicoin.wallet.util.trade_utils import trade_record_to_dict
...
from staicoin.wallet.wallet_node import WalletNode
...
class WalletRpcApi:
    def __init__(self, wallet_node: WalletNode):
        ...
        self.service_name = "staicoin_wallet"
    ...
    async def _state_changed(self, *args) -> List[WsRpcMessage]:
        ...
        return [create_payload_dict("state_changed", data, "staicoin_wallet", "wallet_ui")]
    ...
    async def create_new_wallet(self, request: Dict):
        ...
        if request["wallet_type"] == "cc_wallet":
            ...
            if request["mode"] == "new":
                ...
                from staicoin.pools.pool_wallet_info import initial_pool_state_from_dict
