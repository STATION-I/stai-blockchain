...
def get_staicoin_ca_crt_key() -> Tuple[Any, Any]:
    crt = pkg_resources.resource_string(__name__, "staicoin_ca.crt")
    key = pkg_resources.resource_string(__name__, "staicoin_ca.key")
    return crt, key
...
def generate_ca_signed_cert(ca_crt: bytes, ca_key: bytes, cert_out: Path, key_out: Path):
    ...
    new_subject = x509.Name(
        [
            x509.NameAttribute(NameOID.COMMON_NAME, "staicoin"),
            x509.NameAttribute(NameOID.ORGANIZATION_NAME, "staicoin"),
            x509.NameAttribute(NameOID.ORGANIZATIONAL_UNIT_NAME, "Staicoin Network"),
        ]
    )
    ...
    cert = (
        ...
        .add_extension(
            x509.SubjectAlternativeName([x509.DNSName("staicoin-network.net")]),
            critical=False,
        )
...
def make_ca_cert(cert_path: Path, key_path: Path):
    ...
    subject = issuer = x509.Name(
        [
            x509.NameAttribute(NameOID.ORGANIZATION_NAME, "staicoin"),
            x509.NameAttribute(NameOID.COMMON_NAME, "staicoin CA"),
...
def main():
    return make_ca_cert(Path("./staicoin_ca.crt"), Path("./staicoin_ca.key"))
