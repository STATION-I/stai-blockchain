...
from staicoin import __version__
from staicoin.consensus.coinbase import create_puzzlehash_for_pk
from staicoin.ssl.create_ssl import generate_ca_signed_cert, get_staicoin_ca_crt_key, make_ca_cert
from staicoin.util.bech32m import encode_puzzle_hash
from staicoin.util.config import (
    create_default_staicoin_config,
    initial_config_file,
    load_config,
    save_config,
...
from staicoin.wallet.derive_keys import master_sk_to_pool_sk, master_sk_to_wallet_sk
...
def check_keys(new_root: Path) -> None:
    ...
    if len(all_sks) == 0:
        print("No keys are present in the keychain. Generate them with 'staicoin keys generate'")
        return None
    ...
    for i in range(number_of_ph_to_search):
        ...
        for sk, _ in all_sks:
            ...
            if all_targets[-1] == config["farmer"].get("stai_target_address"):
                stop_searching_for_farmer = True
            if all_targets[-1] == config["pool"].get("stai_target_address"):
                stop_searching_for_pool = True

    # Set the destinations
    if "stai_target_address" not in config["farmer"]:
        print(f"Setting the stai destination address for coinbase fees reward to {all_targets[0]}")
    ...
    if "stai_target_address" not in config["pool"]:
        print(f"Setting the stai destination address for coinbase reward to {all_targets[0]}")
...
def create_all_ssl(root: Path):
    ...
    staicoin_ca_crt, staicoin_ca_key = get_staicoin_ca_crt_key()
    staicoin_ca_crt_path = ca_dir / "staicoin_ca.crt"
    staicoin_ca_key_path = ca_dir / "staicoin_ca.key"
    staicoin_ca_crt_path.write_bytes(staicoin_ca_crt)
    staicoin_ca_key_path.write_bytes(staicoin_ca_key)
    ...
    staicoin_ca_crt, staicoin_ca_key = get_staicoin_ca_crt_key()
    generate_ssl_for_nodes(ssl_dir, staicoin_ca_crt, staicoin_ca_key, False, overwrite=False)
...
def init(create_certs: Optional[Path], root_path: Path):
    if create_certs is not None:
        if root_path.exists():
            ...
            if staicoin_init(root_path) == 0 and root_path.exists():
                return init(create_certs, root_path)

            print(f"** {root_path} was not created. Exiting **")
            return -1
    else:
        return staicoin_init(root_path)


def staicoin_version_number() -> Tuple[str, str, str, str]:
...
def staicoin_minor_release_number():
    res = int(staicoin_version_number()[2])
    print(f"Install release number: {res}")
    return res


def staicoin_full_version_str() -> str:
    major, minor, patch, dev = staicoin_version_number()
    return f"{major}.{minor}.{patch}{dev}"


def staicoin_init(root_path: Path):
    if os.environ.get("staicoin_ROOT", None) is not None:
        print(
            f"warning, your staicoin_ROOT is set to {os.environ['staicoin_ROOT']}. "
            f"Please unset the environment variable and run staicoin init again\n"
            f"or manually migrate config.yaml"
        )

    print(f"staicoin directory {root_path}")
    if root_path.is_dir() and Path(root_path / "config" / "config.yaml").exists():
        # This is reached if staicoin_ROOT is set, or if user has run staicoin init twice
        # before a new update.
    ...
    check_keys(root_path)
    print("")
    print("To see your keys, run 'staicoin keys show --show-mnemonic-seed'")
