...
from staicoin.cmds.init_funcs import staicoin_full_version_str
from staicoin.protocols.protocol_message_types import ProtocolMessageTypes
from staicoin.protocols.shared_protocol import Capability, Handshake
...
from staicoin.util.network import class_for_type, is_localhost

# Max size 2^(8*4) which is around 4GiB
LENGTH_BYTES: int = 4


class WSstaicoinConnection:
    """
    ...
    async def perform_handshake(self, network_id: str, protocol_version: str, server_port: int, local_type: NodeType):
        if self.is_outbound:
            ...
            if inbound_handshake.network_id != 'staicoin-' + network_id:
