...
from staicoin.cmds.init_funcs import staicoin_init
from staicoin.daemon.windows_signal import kill
from staicoin.server.server import ssl_context_for_root, ssl_context_for_server
from staicoin.ssl.create_ssl import get_mozilla_ca_crt
from staicoin.util.staicoin_logging import initialize_logging
from staicoin.util.config import load_config
from staicoin.util.json_util import dict_to_json_str
from staicoin.util.path import mkdir
from staicoin.util.service_groups import validate_service
from staicoin.util.setproctitle import setproctitle
from staicoin.util.ws_message import WsRpcMessage, create_payload, format_response
...
try:
    ...
    print("Error: Make sure to run . ./activate from the project folder before starting staicoin.")
    quit()
...
service_plotter = "staicoin plots create"
...
if getattr(sys, "frozen", False):
    name_map = {
        "staicoin": "staicoin",
        "staicoin_wallet": "start_wallet",
        "staicoin_full_node": "start_full_node",
        "staicoin_harvester": "start_harvester",
        "staicoin_farmer": "start_farmer",
        "staicoin_introducer": "start_introducer",
        "staicoin_timelord": "start_timelord",
        "staicoin_timelord_launcher": "timelord_launcher",
        "staicoin_full_node_simulator": "start_simulator",
    }
...
class WebSocketServer:
    ...
    async def exit(self) -> Dict[str, Any]:
        ...
        log.info("staicoin daemon exiting in 5 seconds")
...
def launch_plotter(root_path: Path, service_name: str, service_array: List[str], id: str):
    # we need to pass on the possibly altered staicoin_ROOT
    os.environ["staicoin_ROOT"] = str(root_path)
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service with staicoin_ROOT: {os.environ['staicoin_ROOT']}")
...
async def async_run_daemon(root_path: Path) -> int:
    staicoin_init(root_path)
    config = load_config(root_path, "config.yaml")
    setproctitle("staicoin_daemon")
