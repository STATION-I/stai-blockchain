...
const PY_FOLDER = '../staicoin/daemon';
...
const getstaicoinVersion = () => {
  let version = null;
  const exePath = getExecutablePath('staicoin');
  // first see if we can get a staicoin exe in a standard location relative to where we are
  try {
    ...
    // that didn't work, let's try as if we're in the venv or staicoin is on the path
    try {
      version = child_process
