...
export default function Block() {
  ...
  const poolReward = mojo_to_staicoin(calculatePoolReward(blockRecord.height));
  const baseFarmerReward = mojo_to_staicoin(
    calculateBaseFarmerReward(blockRecord.height),
  );

  const staicoinFees = blockRecord.fees
    ? mojo_to_staicoin(BigInt(blockRecord.fees))
    : '';
  ...
  const rows = [
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://alltheblocks.net/staicoin/puzzlehash/${blockRecord.farmer_puzzle_hash}`}
        >
          {currencyCode
            ? toBech32m(
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://alltheblocks.net/staicoin/puzzlehash/${blockRecord.pool_puzzle_hash}`}
        >
          {currencyCode
            ? toBech32m(
    ...
    },
    {
      ...
      value: staicoinFees ? `${staicoinFees} ${currencyCode}` : '',
      tooltip: (
        <Trans>
    ...
    <LayoutMain title={<Trans>Block</Trans>}>
      <Card
        title={
          <BlockTitle>
            <Trans>
              Block at height {blockRecord.height} in the Staicoin Blockchain
            </Trans>
          </BlockTitle>
        }
        action={
