...
import { AlertDialog } from '@staicoin/core';
import {
  Typography,
  Button,
  Box,
...
import { staicoin_to_mojo } from '../../../util/staicoin';
...
export const CreateRLAdminWallet = () => {
  ...
  function create() {
    ...
    if (
      staicoinper_input.value === '' ||
      Number(staicoinper_input.value) === 0 ||
      !Number(staicoinper_input.value) ||
      isNaN(Number(staicoinper_input.value))
    ) {
      dispatch(
        openDialog(
          <AlertDialog>
    ...
    const staicoinper = staicoin_to_mojo(staicoinper_input.value);
    const staicoinper_value = Number.parseInt(Number(staicoinper));
    const userpubkey = userpubkey_input.value;
    const amount = staicoin_to_mojo(amount_input.value);
    const amount_value = Number.parseInt(Number(amount));
    // var fee = staicoin_to_mojo(fee_input.value);
