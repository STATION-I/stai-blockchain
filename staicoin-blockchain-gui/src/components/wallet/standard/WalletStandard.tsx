...
import {
  ...
  Fee,
  Form,
  TextField as StaicoinTextField,
  AlertDialog,
  CopyToClipboard,
  Flex,
  Card,
...
import { /* mojo_to_staicoin_string, */ staicoin_to_mojo } from '../../../util/staicoin';
  ...
  return (
    <Grid item xs={12}>
      <Box display="flex">
        ...
        <Box>
          <Typography variant="subtitle1">
            {mojo_to_staicoin_string(props.balance)} {currencyCode}
          </Typography>
        </Box>
      </Box>
    </Grid>
  ...
  return (
    <Card title={<Trans>Balance</Trans>}>
      <BalanceCardSubSection
        ...
        tooltip={
          <Trans>
            This is the total amount of staicoin in the blockchain at the current
      ...
      <BalanceCardSubSection
        ...
        tooltip={
          <Trans>
            This is the amount of staicoin that you can currently use to make
            transactions. It does not include pending farming rewards, pending
            incoming transactions, and staicoin that you have just spent but is not
...
function SendCard(props: SendCardProps) {
  ...
  function handleSubmit(data: SendTransactionData) {
    ...
    if (address.includes('colour')) {
      dispatch(
        openDialog(
          <AlertDialog>
            <Trans>
              Error: Cannot send staicoin to coloured address. Please enter a staicoin
              address.
            </Trans>
          </AlertDialog>,
        ),
    ...
    if (address.slice(0, 12) === 'staicoin_addr://') {
    ...
    const amountValue = Number.parseFloat(staicoin_to_mojo(amount));
    const feeValue = Number.parseFloat(staicoin_to_mojo(fee));
