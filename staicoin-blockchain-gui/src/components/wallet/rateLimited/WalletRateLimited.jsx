...
import { AlertDialog, Card, Flex } from '@staicoin/core';
import {
  send_transaction,
...
const IncompleteCard = (props) => {
  ...
  function submit() {
    ...
    const staicoinper_input = ip_parsed.limit;
    ...
    const staicoinper_value = Number.parseInt(Number(staicoinper_input));
        ...
        <Grid item xs={12}>
          <Box display="flex">
            ...
            <Box flexGrow={1}>
              <Typography variant="subtitle1">
                <Trans>
                  Spending Limit (staicoin per interval):{' '}
                  {mojo_to_staicoin_string(limit)}
                </Trans>
              </Typography>
            </Box>
          </Box>
...
const SendCard = (props) => {
  ...
  function send() {
    ...
    const fee = staicoin_to_mojo(fee_input.value);
