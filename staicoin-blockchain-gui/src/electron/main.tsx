...
import staicoinEnvironment from '../util/staicoinEnvironment';
import staicoinConfig from '../util/config';
...
if (!handleSquirrelEvent()) {
  ...
  const ensureCorrectEnvironment = () => {
    ...
    if (!staicoinEnvironment.guessPackaged() && !('VIRTUAL_ENV' in process.env)) {
  ...
  if (ensureSingleInstance() && ensureCorrectEnvironment()) {
    ...
    staicoinConfig.loadConfig('mainnet');
    ...
    const createWindow = () => {
      ...
      if (!staicoinConfig.manageDaemonLifetime()) {
      ...
      mainWindow.on('close', (e) => {
        ...
        if (decidedToClose || !staicoinConfig.manageDaemonLifetime()) {
          return;
        }
        e.preventDefault();
        if (!isClosing) {
    ...
    const appReady = async () => {
      ...
      // if the daemon isn't local we aren't going to try to start/stop it
      if (staicoinConfig.manageDaemonLifetime()) {
        staicoinEnvironment.startstaicoinDaemon();
      }
    };
  ...
  const getMenuTemplate = () => {
    ...
    if (process.platform === 'darwin') {
      // Staicoin Blockchain menu (Mac)
      template.unshift({
        label: i18n._(/* i18n */ { id: 'staicoin' }),
        submenu: [
          {
            label: i18n._(/* i18n */ { id: 'About Staicoin Blockchain' }),
            click: () => {
              openAbout();
            },
          },
