...
from stai.cmds.keys import delete_all_cmd, generate_and_print_cmd, show_cmd, sign_cmd, verify_cmd
from stai.util.config import load_config
from stai.util.file_keyring import FileKeyring
from stai.util.keychain import DEFAULT_USER, DEFAULT_SERVICE, Keychain, generate_mnemonic
from stai.util.keyring_wrapper import DEFAULT_KEYS_ROOT_PATH, KeyringWrapper, LegacyKeyring
...
class TestKeysCommands:
    ...
    def test_migration_not_needed(self, tmp_path, setup_keyringwrapper, monkeypatch):
        """
        Test the `stai keys migrate` command when no migration is necessary
        """
    ...
    def test_migration_full(self, tmp_path, setup_legacy_keyringwrapper):
        """
        Test the `stai keys migrate` command when a full migration is needed
        """
        ...
        current_version_str = pkg_resources.get_distribution("stai-blockchain").version
    ...
    def test_migration_incremental(self, tmp_path, keyring_with_one_key, monkeypatch):
        ...
        from stai.util import keyring_wrapper
    ...
    def test_migration_checked(self, tmp_path, monkeypatch):
        ...
        assert pkg_resources.get_distribution("stai-blockchain").version == "1.2.11.dev123"
