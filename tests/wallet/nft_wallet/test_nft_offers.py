...
async def test_nft_offer_with_fee(two_wallet_nodes: Any, trusted: Any) -> None:
    ...
    metadata = Program.to(
        [
            ("u", ["https://www.stainetwork.org/img/branding/stai-logo.svg"]),
    ...
    success, trade_make, error = await trade_manager_maker.create_offer_for_ids(
        offer_nft_for_stai, driver_dict, fee=maker_fee
    )
    ...
    await time_out_assert(15, wallet_maker.get_confirmed_balance, maker_balance_pre + stai_request - maker_fee)
    await time_out_assert(15, wallet_taker.get_confirmed_balance, taker_balance_pre - stai_request - taker_fee)
    ...
    success, trade_make, error = await trade_manager_maker.create_offer_for_ids(
        offer_stai_for_nft, driver_dict_to_buy, fee=maker_fee
    )
    await asyncio.sleep(1)
    assert success is True
    assert error is None
    ...
    await time_out_assert(15, get_trade_and_status, TradeStatus.CONFIRMED, trade_manager_maker, trade_make)
    await time_out_assert(15, get_trade_and_status, TradeStatus.CONFIRMED, trade_manager_taker, trade_take)
    await time_out_assert(15, wallet_maker.get_confirmed_balance, maker_balance_pre - stai_offered - maker_fee)
    await time_out_assert(15, wallet_taker.get_confirmed_balance, taker_balance_pre + stai_offered - taker_fee)
...
async def test_nft_offer_nft_for_nft(two_wallet_nodes: Any, trusted: Any) -> None:
    ...
    metadata_2 = Program.to(
        [
            ("u", ["https://www.stainetwork.org/image2.html"]),
