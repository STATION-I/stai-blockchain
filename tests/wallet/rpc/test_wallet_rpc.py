...
async def test_get_transactions(wallet_rpc_environment: WalletRpcTestEnvironment):
    ...
    await client.send_transaction(
        "1", uint64(1), encode_puzzle_hash(await wallet.get_new_puzzlehash(), "tstai")
    )  # Create a pending tx
    ...
    await client.send_transaction("1", uint64(1), encode_puzzle_hash(ph_by_addr, "tstai"))
...
async def test_offer_endpoints(wallet_rpc_environment: WalletRpcTestEnvironment):
    ...
    assert summary == {"offered": {"stai": 5}, "requested": {cat_asset_id.hex(): 1}, "infos": driver_dict, "fees": 1}
...
async def test_nft_endpoints(wallet_rpc_environment: WalletRpcTestEnvironment):
    ...
    from stai.wallet.nft_wallet.nft_info import NFT_HRP
    ...
    res = await wallet_1_rpc.mint_nft(
        ...
        ["https://www.stainetwork.org/img/branding/stai-logo.svg"],
    )
    assert res["success"]
