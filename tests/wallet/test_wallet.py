...
from stai.protocols.full_node_protocol import RespondBlock
...
from stai.wallet.util.wallet_types import AmountWithPuzzlehash
...
class TestWalletSimulator:
    ...
    async def test_wallet_coinbase(
        self,
        wallet_node_sim_and_wallet: Tuple[List[FullNodeSimulator], List[Tuple[WalletNode, StaiServer]]],
        trusted: bool,
        self_hostname: str,
    ) -> None:
        num_blocks = 10
    ...
    async def test_wallet_make_transaction(
        self,
        two_wallet_nodes: Tuple[List[FullNodeSimulator], List[Tuple[WalletNode, StaiServer]]],
        trusted: bool,
        self_hostname: str,
    ) -> None:
        num_blocks = 5
    ...
    async def test_wallet_send_to_three_peers(
        self,
        three_sim_two_wallets: Tuple[List[FullNodeSimulator], List[Tuple[WalletNode, StaiServer]]],
        trusted: bool,
        self_hostname: str,
    ) -> None:
        num_blocks = 10
    ...
    async def test_wallet_make_transaction_hop(
        self,
        two_wallet_nodes_five_freeze: Tuple[List[FullNodeSimulator], List[Tuple[WalletNode, StaiServer]]],
        trusted: bool,
        self_hostname: str,
    ) -> None:
        num_blocks = 10
    ...
    #         outbound: List[WSStaiConnection] = wallet.server.get_outgoing_connections()
    ...
    async def test_address_sliding_window(
        self,
        wallet_node_100_pk: Tuple[List[FullNodeSimulator], List[Tuple[WalletNode, StaiServer]]],
        trusted: bool,
        self_hostname: str,
    ) -> None:
...
def test_get_wallet_db_path_v2_r1() -> None:
    root_path: Path = Path("/x/y/z/.stai/mainnet")
    config: Dict[str, Any] = {
    ...
    assert wallet_db_path == Path("/x/y/z/.stai/mainnet/wallet/db/blockchain_wallet_v2_r1_mainnet_1234567890.sqlite")
...
def test_get_wallet_db_path_testnet() -> None:
    root_path: Path = Path("/x/y/z/.stai/testnet")
    config: Dict[str, Any] = {
        "database_path": "wallet/db/blockchain_wallet_v2_CHALLENGE_KEY.sqlite",
    ...
    assert wallet_db_path == Path("/x/y/z/.stai/testnet/wallet/db/blockchain_wallet_v2_r1_testnet_1234567890.sqlite")
