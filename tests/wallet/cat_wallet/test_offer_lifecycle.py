...
from stai.wallet.trading.offer import Offer, NotarizedPayment
...
class TestOfferLifecycle:
    ...
    async def test_complex_offer(self, setup_sim):
        ...
        try:
            ...
            stai_announcements: List[Announcement] = Offer.calculate_announcements(stai_requested_payments)
            stai_secured_bundle: SpendBundle = generate_secure_bundle(stai_coins, stai_announcements, 1000)
            stai_offer = Offer(stai_requested_payments, stai_secured_bundle)
            ...
            new_offer = Offer.aggregate([stai_offer, red_offer])
