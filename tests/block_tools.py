...
from staicoinbip158 import PyBIP158

from staicoin.cmds.init_funcs import create_all_ssl, create_default_staicoin_config
from staicoin.full_node.bundle_tools import (
    best_solution_generator_from_template,
    detect_potential_template_generator,
    simple_solution_generator,
)
from staicoin.util.errors import Err
from staicoin.full_node.generator import setup_generator_args
from staicoin.full_node.mempool_check_conditions import GENERATOR_MOD
from staicoin.plotting.create_plots import create_plots
from staicoin.consensus.block_creation import unfinished_block_to_full_block
...
from staicoin.consensus.blockchain_interface import BlockchainInterface
from staicoin.consensus.coinbase import create_puzzlehash_for_pk, create_farmer_coin, create_pool_coin
...
from staicoin.consensus.deficit import calculate_deficit
from staicoin.consensus.full_block_to_block_record import block_to_block_record
from staicoin.consensus.make_sub_epoch_summary import next_sub_epoch_summary
from staicoin.consensus.cost_calculator import NPCResult, calculate_cost_of_program
from staicoin.consensus.pot_iterations import (
    calculate_ip_iters,
...
from staicoin.consensus.vdf_info_computation import get_signage_point_vdf_info
from staicoin.full_node.signage_point import SignagePoint
from staicoin.plotting.plot_tools import PlotInfo, load_plots, parse_plot_info
...
from staicoin.types.blockchain_format.foliage import Foliage, FoliageBlockData, FoliageTransactionBlock, TransactionsInfo
...
from staicoin.types.blockchain_format.reward_chain_block import RewardChainBlockUnfinished
from staicoin.types.blockchain_format.sized_bytes import bytes32
from staicoin.types.blockchain_format.slots import (
...
from staicoin.types.generator_types import BlockGenerator, CompressorArg
...
from staicoin.util.condition_tools import ConditionOpcode, conditions_by_opcode
from staicoin.util.config import load_config, save_config
from staicoin.util.hash import std_hash
from staicoin.util.ints import uint8, uint16, uint32, uint64, uint128
from staicoin.util.keychain import Keychain, bytes_to_mnemonic
from staicoin.util.merkle_set import MerkleSet
from staicoin.util.prev_transaction_block import get_prev_transaction_block
from staicoin.util.path import mkdir
...
class BlockTools:
    ...
    def __init__(
        ...
        create_default_staicoin_config(root_path)
        ...
        if len(self.pool_pubkeys) == 0 or len(self.farmer_pubkeys) == 0:
            raise RuntimeError("Keys not generated. Run `staicoin generate keys`")
    ...
    def init_plots(self, root_path: Path):
        ...
        try:
            ...
            args.pool_contract_address = encode_puzzle_hash(self.pool_ph, "stai")
...
def get_plot_dir() -> Path:
    cache_path = Path(os.path.expanduser(os.getenv("staicoin_ROOT", "~/.staicoin/"))) / "test-plots"
    mkdir(cache_path)
    return cache_path
