...
from stai.cmds.init_funcs import create_all_ssl, create_default_stai_config
from stai.daemon.keychain_proxy import connect_to_keychain_and_validate, wrap_local_keychain
...
from stai.full_node.generator import setup_generator_args
from stai.full_node.mempool_check_conditions import GENERATOR_MOD
from stai.plotting.create_plots import create_plots, PlotKeys
...
from stai.consensus.coinbase import create_puzzlehash_for_pk, create_farmer_coin, create_pool_coin
...
from stai.plotting.util import PlotsRefreshParameter, PlotRefreshResult, PlotRefreshEvents, parse_plot_info
...
from stai.util.config import load_config, lock_config, save_config, override_config
    ...
    async def setup_keys(self):
        if self.local_keychain:
            ...
            self.keychain_proxy = await connect_to_keychain_and_validate(
                self.root_path, log, user="testing-1.8.0", service="stai-testing-1.8.0"
            )
        ...
        if len(self.pool_pubkeys) == 0 or len(self.farmer_pubkeys) == 0:
            raise RuntimeError("Keys not generated. Run `stai generate keys`")
