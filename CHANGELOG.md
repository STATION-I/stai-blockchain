...
## 1.2.0 staicoin blockchain 2021-07-07
...
- All staicoin repositories now automatically build M1 wheels and create a new MacOS M1 native installer.
- New CLI command `staicoin plotnft` to manage pools.
...
- Correctly display private key in `staicoin keys show`.
- Thanks to @gldecurtins for removing a default printout of the private key mnemonic in `staicoin keys show`.
...
- Thank you to @staicoinMineJP for various improvements.
...
- `staicoin farm summary` better handles wallet errors.
...
### 1.1.7 staicoin Blockchain 2021-06-05
...
## 1.1.6 staicoin Blockchain 2021-05-20
...
- We added a simple profiler to track performance times in the application (see /staicoin/util/profiler.py for instructions).
...
- Added configuration options for moving an install to testnet and back (use `staicoin configure -t true|false`).
...
## 1.1.5 staicoin Blockchain 2021-05-09
...
## 1.1.4 staicoin Blockchain 2021-05-04

### Changed

- Secret wallet key is hidden by default in `staicoin keys show`. Use `staicoin keys show --show-mnemonic-seed` for private keys.
...
## 1.1.3 staicoin Blockchain 2021-05-01
...
## 1.1.2 staicoin Blockchain 2021-04-24
...
## 1.1.1 staicoin Blockchain 2021-04-21
...
## 1.1.0 staicoin Blockchain 2021-04-21

### Added

- This fork release includes full transaction support for the staicoin Blockchain. Transactions are still disabled until 5/3/2021 at 10:00AM PDT. It is hard to overstate how much work and clean up went into this release.
...
## 1.0.5 staicoin Blockchain 2021-04-14
...
## 1.0.4 staicoin Blockchain 2021-04-12
...
- The `src` directory in staicoin-blockchain has been changed to `staicoin` to avoid namespace collisions.
- GUI install builds have been simplified to rely on one `.spec` file in `staicoin/`
...
## 1.0.3 staicoin Blockchain 2021-03-30
...
- An incorrect merge brought in unreleased features and broke `staicoin keys`.
- Omitted from the 1.0.2 changelog, we fixed one crash in harvester with the release of chiapos 1.0.0 as well.

## 1.0.2 staicoin Blockchain 2021-03-30
...
- @jespino changed `staicoin keys show` to require the `--show-mnemonic-seed` before it displays your 24 work private key mnemonic.
- We decreased the size of the block cache in node to perform better with longer chains.
- You can now add a private key mnemonic from a file with `staicoin keys show`.
...
## 1.0.1 staicoin Blockchain 2021-03-23
...
- Users can now pip install e.g. staicoin-blockchain==1.0.1 on most platforms.
- Sometimes the GUI had an error regarding MainWindow.

## 1.0.0 First Release of staicoin Blockchain 2021-03-17

### Added

- This is the first production release of the staicoin Blockchain. This can be installed and will wait for the green flag that will be dropped at approximately 7AM PDST (14:00 UTC) on Friday March 19, 2021. All farming rewards from that point forward will be considered valid and valuable stai. There is a six week lock on all transactions. During those six weeks farmers will be earning their farming rewards but those rewards can not be spent.
...
- Migration with STAICOIN_ROOT set does not crash staicoin init

## 1.0rc8 aka Release Candidate 8 - 2021-03-15

### Added

- This is a hard fork/breaking change from RC6/7. Tstai Coins will **not** be moved forward but your plots and keys and parts of your configuration do. When you install this version before 10AM PDST on 3/16/2021 it will load up, start finding peers, and otherwise wait for the flag drop at that time to start farming. This is likely to be the last dress rehearsal for mainnet launch. Our [3/15/2021 blog post](https://www.staicoin-network.net/2021/03/15/mainnet-update.html) has more details on the current mainnet launch plan.
...
- `staicoin farm summary` will now display Tstai or stai as appropriate.
...
- `staicoin farm summary` displayed some incorrect amounts.
...
- `staicoin netspace` now defaults to 1000 blocks to mirror the GUI.
...
- Remove `staicoin plots "-s" "--stripe_size"` and the strip size setting in the Advanced section of the GUI. We now always use the best default of 64K for the GUI and cli.
- `staicoin keys add` takes secret words a prompt on the command line or stdin instead of command line arguments for security.
...
- We have moved to a new and much higher scalability download.staicoin-network.net to support the mainnet launch flag and additional download demand.
- To always get the latest testnet and then mainnet installers you can now use a latest URL: [Windows](https://download.staicoin-network.net/latest/Setup-Win64.exe) and [MacOS x86_64](https://download.staicoin-network.net/latest/Setup-MacOS.dmg).
- staicoin wheels not on Pypi and some dependecies not found there also are now on pypi.chia.net.
...
- PRs and commits to the staicoin-blockchain-gui repository will automatically have their locales updated.

## Fixed

- The Farm page will now no longer get stuck at 50 Tstai farmed.
- `staicoin farm` has had multiple bugs and spelling issues addressed. Thanks to @alfonsoperez, @soulmerge and @olivernyc for your contributions.
- `staicoin wallet` had various bugs.
...
- We believe that the install/migrate process was bringing forward bad farming rewards receive addresses. We have attempted to stop that by only migrating RC3 and newer configurations. You can make sure you are not effected by using the Manage Farming Rewards tool mentioned above or putting a known good wallet receive address in both `stai_target_address` sections of config.yaml.
...
- The RC5 release is a new breaking change/hard fork blockchain. Plots and keys from previous chains will work fine on RC5 but balances of Tstai will not come forward.
- We now support a "green flag" chain launch process. A new version of the software will poll download.staicoin-network.net/notify/ for a signed json file that will be the genesis block of the chain for that version. This will allow unattended start at mainnet.
...
- Thanks to @jespino there is now `staicoin farm summary`. You can now get almost exactly the same farming information on the CLI as the GUI.
- We have added Romanian to the GUI translations. Thank you to @bicilis on [Crowdin](https://crowdin.com/project/staicoin-blockchain). We also added a couple of additional target languages. Klingon anyone?
- `staicoin wallet` now takes get_address to get a new wallet receive address from the CLI.
- `staicoin plots check` will list out all the failed plot filenames at the end of the report. Thanks for the PR go to @eFishCent.
...
- Thanks to @xdustinface of Dash, the BlS Signature library has been updated to 0.9 with clean ups and some speed ups. This changed how the G2 infinity element was handled and we now manage it inside of staicoin-blockchain, etc., instead of in blspy.
...
- And thanks yet again to @jespino for a host of PRs to add more detailed typing to various components in staicoin-blockchain.
...
- Adding Click changed the behavior of `staicoin keys add -m`. The help now makes it clear that the 24 word mnemonic needs to be surrounded by a pair of quotes.
- Python root CA certificates have issues so we have added the Mozilla certificate store via curl.se and use that to connect to backup.staicoin-network.net via https, for example.
- The difficulty adjustment calculation was simplified.
- All of the staicoin sub repositories that were attempting to build MacOS Universal wheels were only generating x86_64 wheels internally. We have moved back to only generating x86_64 MacOS wheels on CI.
- However, we have updated and test compiled all staicoin dependencies on Apple Silicon and will be making available a test .dmg for MacOS ARM64 shortly.
- Various weight proof edge cases have been fixed.
- Various typos and style clean ups were made to the Click CLI implementation. `staicoin -upnp f` was added to disable uPnP.
- `staicoin plots check` shouldn't crash when encountering plots that cause RuntimeError. PR again thanks to @eFishCent.
...
- RC3 is a new chain to support the last major chialisp changes. Tstai from the RC1/2 chain do not come forward to this chain but plots and keys continue to work as usual.
...
- There are now Swedish, Spanish, and Slovak translations. Huge thanks to @ordtrogen (Swedish), @jespino and @dvd101x (Spanish), and our own @seeden (Slovak). Also thanks were due to @f00b4r (Finnish), @A-Caccese (Italian), and @Bibop182 and @LeonidShamis (Russian). Quite a few more are almost complete and ready for inclusion. You can help translate and review translations at our [crowdin project](https://crowdin.com/project/staicoin-blockchain).
- You can obtain a new wallet receive address on the command line with `staicoin wallet new_address`. Thanks to @jespino for this and a lot more in the next section below.
...
- The entire CLI has been ported to [Click](https://click.palletsprojects.com/en/7.x/). Huge thanks to @jespino for the big assist and @unparalleled-js for the [recommendation and the initial start](https://github.com/pinksheetscrypto/staicoin-blockchain/issues/464). This will make building out the CLI much easier. There are some subtle changes and some shortcuts are not there anymore. `staicoin -h` and `staicoin SUBCOMMAND -h` can be your guide.
...
- `staicoin plots check` now will continue checking after it finds an error in a plot to the total number of checks you specified.
...
- Interface improvements for `staicoin netspace`.
- Now that aiosqlite included our upstream improvements we install version 0.17.0.
- `staicoin init` only migrates release candidate directories. The versioned sub directories under `~/staicoin` will be going away before mainnet.
...
- `staicoin configure -log-level` was broken.
...
- This is an errata release for Release Candidate 1. There were a couple of things that did not smoothly migrate from the Beta versions. Please make sure you also consult the [release notes for RC-1](https://github.com/pinksheetscrypto/staicoin-blockchain/releases/tag/1.0rc1) was well.
...
- Installers will now be of the pattern staicoinSetup-0.2.1.exe. `0.2` is release candidate and the final `.1` is the first release candidate.
- Use 'staicoin wallet get_transactions' in the command line to see your transactions.
- 'staicoin wallet show' now shows your wallet's height.
...
- We have integrated with crowdin to make it easier to translate the GUI. Check out [staicoin Blockchain GUI](https://crowdin.com/project/staicoin-blockchain) there.
- We have added Italian, Russian, and Finnish. More to come soon.
- There is now remote UI support. [Documents](https://github.com/pinksheetscrypto/staicoin-blockchain-gui/blob/main/remote.md) will temporarily live in the repository but have moved to the [wiki](https://github.com/pinksheetscrypto/staicoin-blockchain/wiki/Connecting-the-UI-to-a-remote-daemon). Thanks to @dkackman for this excellent addition!
...
- Added `staicoin plots check --challenge-start [start]` that begins at a different `[start]` for `-n [challenges]`. Useful when you want to do more detailed checks on plots without restarting from lower challenge values you already have done. Huge thanks to @eFishCent for this and all of the debugging work behind the scenes confirming that plot failures were machine errors and not bugs!
...
- 'staicoin wallet send' now takes in Tstai or stai as units instead of mojos.
...
- The Beta 27 chain is a hard fork. All Tstai from previous releases has been reset on this chain. Your keys and plots of k=32 or larger continue to work just fine on this new chain.
...
- You can now send and receive transactions with the command line. Try `staicoin wallet -h` to learn more. Also, `staicoin wallet` now requires a third argument of `show`, therefor you will use `staicoin wallet show` to see your wallet balance.
- We have added the [Crowdin](https://crowdin.com/) translation platform to [staicoin blockchain gui](https://crowdin.com/project/staicoin-blockchain). We are still getting it fully set up, but helping to translate the GUI is going to be much easier.
...
- `staicoin init -c [directory]` will create new TLS certificates signed by your CA located in `[directory]`. Use this feature to configure a new remote harvester. Type `staicoin init -h` to get instructions. Huge thanks to a very efficient @eFishCent for this quick and thorough pull request.
...
- This release has the final mainnet rewards schedule. During the first three years, each block winner will win 2 Tstai/stai per block for a total of 9216 Tstai per day from 4608 challenges per day.
- Smart transactions now use an announcement instead of 'coin consumed' or lock methods.
- The GUI is now in a separate submodule repository from staicoin-blockchain, [staicoin-blockchain-gui](https://github.com/pinksheetscrypto/staicoin-blockchain-gui). The installers and install scripts have been updated and it continues to follow the same install steps. Note that the GUI directory will now be `staicoin-blockchain-gui`. The workflow for this may be "touch and go" for people who use the git install methods over the short term.
...
- The cli wallet, `staicoin wallet`, was incorrectly displaying Tstai balances as if they were Coloured Coins.
...
- `staicoin plots check -l` will list all duplicate plot IDs found on the machine. Thanks very much for this PR @eFishCent.

### Changed

- Significant improvements have been made to how the full node handles the mempool. This generally cuts CPU usage of node by 2x or more. Part of this increase is that we have temporarily limited the size of transactions. If you want to test sending a transaction you should keep the value of your transaction below 20 Tstai as new consensus will cause you to use a lot of inputs. This will be returned to the expected level as soon as the integration of [clvm rust](https://github.com/pinksheetscrypto/clvm_rs) is complete.
- We have changed the way TLS between nodes and between staicoin services work. Each node now has two certificate authorities. One is a public, shared CA that signs the TLS certificates that every node uses to connect to other nodes on 8444 or 1999. You now also have a self generated private CA that must sign e.g. farmer and harvester's certificates. To run a remote harvester you need a new harvester key that is then signed by your private CA. We know this is not easy for remote harvester in this release but will address it quickly.
...
- We have made the "Help Translate" button in the GUI open in your default web browser and added instructions for adding new translations and more phrases in existing translations at that [URL](https://github.com/pinksheetscrypto/staicoin-blockchain/tree/main/electron-react/src/locales). Try the "Help Translate" option on the language selection pull down to the left of the dark/light mode selection at the top right of the GUI.
...
- We are down to only one sub dependency that does not ship binary wheels for all four platforms. The only platform still impacted is ARM64 (generally Raspberry Pi) but that only means that you still need the minor build tools as outlined on the [wiki](https://github.com/pinksheetscrypto/staicoin-blockchain/wiki/Raspberry-Pi).
...
- Last Attempted Proof is now fixed. This will show you the last time one of your plots passed the [plot filter](https://github.com/pinksheetscrypto/staicoin-blockchain/wiki/FAQ#what-is-the-plot-filter-and-why-didnt-my-plot-pass-it).
...
- `staicoin plots check` enforces a minimum of `-n 5` to decrease false negatives. Thanks to @eFishCent for these ongoing pull requests!
...
- We have removed the '-r' flag from `staicoin` as it was being overridden in most cases by the `-r` for restart flag to `staicoin start`. Use `staicoin --root-path` instead.
- `staicoin -h` now recommends `staicoin netspace -d 192` which is approximately one hours worth of sub blocks. Use `-d 1000` to get the same estimate of netspace as the RPC and GUI.
- `staicoin show -c` now displays in MiB and the GUI has been changed to MiB to match.
- `staicoin configure` now accepts the shorter `-upnp` and `-log-level` arguments also.
- `staicoin plots check` now defaults to `-n 30` instead of `-n 1` - HT @eFishCent.
- `staicoin plots create` now enforces a minimum of k=22. As a reminder, anything less than k=32 is just for testing and be careful extrapolating performance of a k less than 30 to a k=32 or larger.
...
- `staicoin show -c` was showing upload MiB in the download column and vice versa. @pyl and @psydafke deserves credit for insisting it was broken and @kd637xx for the PR assist.
- `staicoin show` handles sub block 0 better.
...
- `staicoin plots create -x` skips adding [final dir] to harvester for farming
...
- Removed loading plot file names when starting `staicoin plots create`; decreases plotter time when there are a lot of plots on the machine. Huge thanks to @eFishCent for this PR!
...
- `staicoin show -h` now correctly states that you use the first 8 characters of the node id to remove a node on the cli.
- Thank you to @wallentx for adding better help for `staicoin configure --enable-upnp`.
...
- `staicoin configure` now lets you enable or disable uPnP.
...
- `staicoin netspace -s` would not correctly look up the start block height by block hash. Additionally netspace now flips to PiB above 1024 TiB. To compare netspace to `staicoin show` of the GUI use `staicoin netspace -d 1000` as `staicoin netspace` defaults to `-d 192` which is one hour.
...
- Welcome to the new consensus. This release is an all but a full re-write of the blockchain in under 30 days. There is now only one tip of the blockchain but we went from two chains to three. Block times are now a little under a minute but there are a couple of sub blocks between each transaction block. A block is also itself a special kind of sub block and each sub block rewards the farmer who won it 1 Tstai. Sub blocks come, on average, about every 17 to 18 seconds.
- Starting with this Beta, there are 4608 opportunities per day for a farmer to win 1 Tstai compared to Beta 18 where there were 288 opportunities per day for a farmer to win 16 Tstai.
- There is a lot more information and explanation of the new consensus algorithm in the New Consensus Working Document linked from [staicoin-network.net](https://staicoin-network.net/). Among the improvements this gives the staicoin blockchain are a much higher security level against all attacks, more frequent transaction blocks that have less time variation between them and are then buried under confirmations (sub blocks also count towards re-org security) much more quickly.
- New consensus means this is a very hard fork. All of your Tstai from Beta 17/18 will be gone. Your plots and keys will work just fine however. You will have to sync to the new chain.
...
- Prior to this Beta 19, we had block times that targeted 5 minutes and rewarded 16 Tstai to one farmer. Moving forward we have epoch times that target 10 minutes and reward 32 Tstai to 32 farmers about every 17-18 seconds over that period. This has subtle naming and UI impacts in various places.
...
- You can now specify which private key to use for `staicoin plots create`. After obtaining the fingerprint from `staicoin keys show`, try `staicoin plots create -a FINGERPRINT`. Thanks to @eFishCent for this pull request!
...
- The Farm tab in the GUI is significantly enhanced. Here you have a dashboard overview of your farm and your activity in response to challenges blockchain challnegs, how long it will take you - on average - to win a block, and how much Tstai you've won so far. Harvester and Full Node connections have moved to Advanced Options.
- Harvester and farmer will start when the GUI starts instead of waiting for key selection if there are already keys available. This means you will start farming on reboot if you have the staicoin application set to launch on start.
- Testnet is now running at the primary port of 1999. Update your routers appropriately. This opens 8444 for mainnet.
- All networking code has been refactored and mostly moved to websockets.
- RPCs and daemon now communicate over TLS with certificates that are generated into `~/.staicoin/VERSION/config/`
...
- `staicoin show -s` has been refactored to support the new consensus.
- `staicoin netspace` has been refactored for new consensus.
...
- Tests now place a cache of blocks and plots in the ~/.staicoin/ directory to speed up total testing time.
- Changes were made to chiapos to correctly support the new bitfiled backpropogation on FreeBSD and OpenBSD. With the exception of needing to work around python cryptography as outlined on the wiki, FreeBSD and OpenBSD should be able to compile and run staicoin-blockchain.
...
- `staicoin show -s` now is less brain dead when a node is initially starting to sync.
...
- Partial support for Python 3.9. That includes new versions of staicoin dependencies like chiabip158.
...
- A segfault caused by memory leaks in bls-library has been fixed. This should end the random farmer and harvester crashes over time as outlined in [Issue 500](https://github.com/pinksheetscrypto/staicoin-blockchain/issues/500).
...
- FreeBSD and OpenBSD should be able to build and install staicoin-blockchain and its dependencies again.
...
- In the GUI there was [a regression](https://github.com/pinksheetscrypto/staicoin-blockchain/issues/484) that removed the scroll bar on the Plot page. The scroll bar has returned!
...
- Logging had [a regression](https://github.com/pinksheetscrypto/staicoin-blockchain/issues/485) where log level could no longer be set by service.

## [1.0beta16] aka Beta 1.16 - 2020-10-20

### Added

- The staicoin GUI now supports dark and light mode.
- The GUI now supports translations and localizations. If you'd like to add your language you can see the examples in [the locales directory](https://github.com/pinksheetscrypto/staicoin-blockchain/tree/dev/electron-react/src/locales) of the staicoin-blockchain repository.
- `staicoin check plots` now takes a `-g` option that allows you to specify a matching path string to only check a single plot file, a wild card list of plot files, or all plots in a single directory instead of the default behavior of checking every directory listed in your config.yaml. A big thank you to @eFishCent for this pull request!
...
- The rpc interfaces of all staicoin services have been refactored, simplified, and had various additional functionality added.
- Block timestamps are now stored in the wallet database. Both database versions were incremented and databases from previous versions will not work with Beta 14. However, upon re-sync all test staicoin since Beta 12 should appear in your wallet.
...
- Long_description_content_type is now set to improve staicoin-blockstaicoinn's Pypi entry. Thanks to @altendky for this pull request.
...
- You can now backup your wallet related metadata in an encrypted and signed file to a free service from staicoin Network at backup.staicoin-network.net. Simply having a backup of your private key will allow you to fully restore the state of your wallet including coloured coins, rate limited wallets, distributed identity wallets and many more. Your private key is used to automatically restore the last backup you saved to the staicoin backup cloud service. This service is open source and ultimately you will be able to configure your backups to go to backup.staicoin-network.net, your own installation, or a third party's version of it.
- Added a Code of Conduct in CODE_OF_CONDUCT.md.
- Added a bug report template in `.github/ISSUE_TEMPLATE/bug_report.md`.

### Changed

- This is a new blockchain as we changed how the default puzzle hashes are generated and previous coins would not be easy to spend. Plots made with Beta 8 and newer continue to work, but all previous test staicoin are left on the old chain and do not migrate over. Configuration data like plot directories automatically migrate in your `~/.staicoin` directory.
...
- `staicoin init` now automatically disstaiers previous releases in each new release.

### Fixed

- `staicoin show -w` should now more reliably work. Wallet balances should be more often correct.
...
- Fixed a typo in `staicoin show -w` with thanks to @pyl on Keybase.
- In Windows the start menu item is now staicoin Network and the icon in Add/Remove is updated.

## [1.0beta11] aka Beta 1.11 - 2020-08-24

### Added

- The staicoin UI now has a proper About menu entry that gives the various component versions and directs people to submit issues on GitHub. Thank you to @freddiecoleman for this pull request!
- Ability to run only the farmer, wallet, or timelord services, for more advanced configurations (staicoin run farmer-only, wallet-only, timelord-only)
...
- `staicoin show -w` now correctly outputs all wallet types and balances from the local wallet.
...
- `staicoin show -b` now returns plot public key and pool public key for each block.
- Added cbor2 binary wheels for ARM64 to the staicoin simple site. Raspberry Pi should be just a little easier to install.

### Changed

- Wallet addresses and other key related elements are now expressed in Chech32 which is the staicoin implementation of [Bech32](https://github.com/bitcoin/bips/blob/master/bip-0173.mediawiki). All of your old wallet addresses will be replaced with the new Chech32 addresses. The only thing you can't do is send test staicoin between 1.8/1.9 and 1.10 software. Anyone who upgrades to 1.10 will keep their transactions and balances of test staicoin from the earlier two releases however.
...
- `staicoin show -w` now displays a message when balances cannot be displayed instead of throwing an error. Thanks to @freddiecoleman for this fix!
- Fix issue with shutting down full node (full node processes remained open, and caused a spinner when launching staicoin)
...
- See wallet balances in command line: `staicoin show -w`
- Retry opening invalid plots every 20 minutes (so you can copy a large plot into a plot directory.)
- We've added `staicoin keys sign` and `staicoin keys verify` to allow farmers to certify their ownership of keys.
...
- `staicoin start wallet-server` changed to `staicoin start wallet`, for consistency.
...
- Removed legacy scripts such as staicoin-stop-server, staicoin-restart-harvester, etc.
...
- The staicoin executable is now available if installing from the Windows or MacOS
Graphical installer. Try `./staicoin -h` from
`~\AppData\Local\staicoin-Blockchain\app-0.1.8\resources\app.asar.unpacked\daemon\`
in Windows or
`/Applications/staicoin.app/Contents/Resources/app.asar.unpacked/daemon` on MacOS.

### Changed

- Minor changes have been made across the repositories to better support
compiling on OpenBSD. HT @n1000.
- Changed stai units to Tstai units for testnet.
...
"staicoin plots create" and "staicoin plots check".
- Add plot directories to config.yaml from the cli with "staicoin plots add".
...
- The Windows installer is now signed by a staicoin Network certificate. It may take some time to develop enough reputation to not warn multiple times during install.
...
- Test plots for the simulation and testing harness now go into `~/.staicoin/test-plots/`
- We have completely refactored all networking code towards making each staicoin service use the same default networking infrastructure and move to websockets as the default networking wire protocol.
- We added additional improvements and more RPCs to the start daemon and various services to continue to make staicoin start/stop reliable cross platform.
...
- We now codesign the Apple .dmg installer with the staicoin Network developer ID on both GitHub Actions and Azure Pipelines. We will be notarizing and distributing the Azure Pipelines version as it's built on MacOS Mojave (10.14.6) for stronger cross version support.
...
- Starting staicoin now waits for the full node to be active before contacting the introducer.
...
- You can now create or import private keys with a 24 word mnemonic, both in the UI and 'staicoin keys' command line.
...
- We added a `staicoin keys` command line program, to see, add, and remove private keys.
...
- We added [lgtm](https://lgtm.com/) source analysis on pull request to the staicoin-blockchain, chiapos, chiavdf, chiabip158, and bls-library repositories to add some automated security analysis to our ci.
...
- `staicoin netspace` has been refactored to use the `/get_network_space` RPC. The command
...
- `staicoin-check-plots` uses the plot root and checks the plots in the same manner as harvester.
- `staicoin-check-plots` now does not override plots.yaml, which means concurrent plots will properly be added to plots.yaml.
...
- We have added an option to `staicoin-create-plots` to specify the second temporary directory. Creating a plot is a three step process. First a working file ending in `.dat.tmp` is created. This file is usually 5 times larger than the final plot file. In the later stages of plotting a second temp file is created ending in `.dat.2.tmp` which will grow to the size of the final plot file. In the final step, the `.dat.2.tmp` is copied to the final `.dat` plot file. You can now optionally set the directory for the `.dat.2.tmp` file with the `-2` flag. An example use case is plotting on a ramdisk and writing both the second temp file and the final file out to an SSD - `staicoin-create-plots -n 1 -k 30 -t /mnt/ramdisk -2 /mnt/SSD -d /mnt/SSD`.

### Changed

- `staicoin init` properly migrates from previous versions including the k>=32 workaround. Additionally, the farming target key is checked to make sure that it is the valid and correct public key format.
- We have implemented a workaround for the `staicoin start` issues some were having upon crash or reboot. We will be rebuilding start and stop to be robust across platforms.
- This release re-includes `staicoin-start-harvester`.
...
- The user interface now refers to staicoin coins with their correct currency code of stai.
...
- There was a regression in staicoin Proof of Space ([chiapos](https://github.com/pinksheetscrypto/chiapos)) that came from our efforts to speed up plotting on Windows native. Now k>=32 plots work correctly. We made additional bug fixes and corrected limiting small k size generation.
- There was a bug in Timelord handling that could stop all VDF progress.

### Deprecated

- We have made significant changes to the full node database to make it more reliable and quicker to restart. This requires re-syncing the current chain. If you use `staicoin init` then sync on first start will happen automatically. "\$STAICOIN_ROOT" users will need to delete `$STAICOIN_ROOT/db/*` before starting Beta 1.5. This also fixes the simulation issue in Beta 1.4 where tips could go "back in time."
...
- Now you can run estimates of the total space currently farming the network. Try `staicoin netspace -d 12` to run an estimate over the last 12 blocks which is approximately 1 hour.
- We’ve added TLS authentication for incoming farmer connections. TLS certs and keys are generated during staicoin init and only full nodes with your keys will be able to connect to your Farmer. Also, Harvester, Timelord, and Wallet will now not accept incoming connections which reduces the application attack surface.
- The node RPC has a new endpoint get_header_by_height which allows you to retrieve the block header from a block height. Try `staicoin show -bh 1000` to see the block header hash of block 1000. You can then look up the block details with `staicoin show -b f655e1a9f7f8c89a703e40d9ce82ae33508badaf7b37fa1a56cad27926b5e936` which will look up a block by it's header hash.
...
- Most of the content of README.md and INSTALL.md have been moved to the [repository wiki](https://github.com/pinksheetscrypto/staicoin-blockchain/wiki) and placed in [INSTALL](https://github.com/pinksheetscrypto/staicoin-blockchain/wiki/INSTALL) and [Quick Start Guide](https://github.com/pinksheetscrypto/staicoin-blockchain/wiki/Quick-Start-Guide)
...
- Most scripts have been removed in favor of staicoin action commands. You can run `staicoin version` or `staicoin start node` for example. Just running `staicoin` will show you more options. However `staicoin-create-plots` continues to use the hyphenated form. Also it's now `staicoin generate keys` as another example.
- staicoin start commands like `staicoin start farmer` and `staicoin stop node` now keep track of process IDs in a run/ directory in your configuration directory. `staicoin stop` is unlikely to work on Windows native for now. If `staicoin start -r node` doesn't work you can force the run/ directory to be reset with `staicoin start -f node`.
- We suggest you take a look at our [Upgrading documentation](https://github.com/pinksheetscrypto/staicoin-blockchain/wiki/Updating-beta-software) if you aren't performing a new install.
...
- The script `staicoin start wallet-gui` has been chaned to `staicoin start wallet` which launches but the GUI and server on MacOS and Linux. `staicoin start wallet-server` remains for WSL 2 and Windows native.
...
- Plots of k>=32 are not working for farming, and some broken plots can cause a memory leak. A [workaround is available](https://github.com/pinksheetscrypto/staicoin-blockchain/wiki/Beta-1.4-k=32-or-larger-work-around).
...
- Currently, there is no way to restore a Coloured Coin Wallet.
- The `staicoin stop all` command sometimes fails, use `staicoin-stop-all` instead. In windows, use the task manager to stop the servers.
...
- There is a new command `staicoin init` that will migrate files and generate your initial configuration. If you want to use the Wallet or farm, you will also have to `staicoin-generate-keys`. You can read step by step instructions for [upgrading from a previous beta release](https://github.com/pinksheetscrypto/staicoin-blockchain/wiki/Updating-beta-software). If you've set `$STAICOIN_ROOT` you will have to make sure your existing configuration remains compatible manually.
...
- We added full block lookup to `staicoin show`.

### Changed

- `staicoin-restart-harvester` has been renamed from `staicoin-start-harvester` to better reflect its functionality. Use it to restart a harvester that's farming so that it will pick up newly finished plots.
...
- If you previously used the plot_root variable in config, your plot directory names might not migrate correctly. Please double check the filenames in `~/.staicoin/beta-1.0b3/config/plots.yaml` after migrating
...
- There is now full transaction support on the staicoin blockchain. In this initial Beta 1.0 release, all transaction types are supported though the wallets and UIs currently only directly support basic transactions like coinbase rewards and sending coins while paying fees. UI support for our [smart transactions](https://github.com/pinksheetscrypto/wallets/blob/main/README.md) will be available in the UIs shortly.
...
- We have revamped the staicoin management command line. To start a farmer all you have to do is start the venv with `. ./activate` and then type `staicoin-start-farmer &`. The [README.md](https://github.com/pinksheetscrypto/staicoin-blockchain/blob/main/README.md) has been updated to reflect the new commands.
...
- Configuration directories have been moved to a default location of HomeDirectory/.staicoin/release/config, plots/ db/, wallet/ etc. This can be overridden by `export STAICOIN_ROOT=~/.staicoin` for example which would then put the plots directory in `HomeDirectory/.staicoin/plots`.
- The libraries staicoin-pos, staicoin-fast-vdf, and staicoin-bip-158 have been moved to their own repositories: [chiapos](https://github.com/pinksheetscrypto/chiapos), [chiavdf](https://github.com/pinksheetscrypto/chiavdf), and [chaibip158](https://github.com/pinksheetscrypto/chiabip158). They are brought in by staicoin-blockchain at install time. Our BLS signature library remains at [bls-signatures](https://github.com/pinksheetscrypto/bls-signatures).
...
- `staicoin-regenerate-keys` has been renamed `staicoin-generate-keys`.
...
- staicoin fullnode, farmer and harvester now default to logging to staicoin.log in the staicoin-blockchain directory. This is configured in config.yaml and due to config.yaml changes it is recommended to edit the new template config instead of using older config.yaml’s from previous versions.
...
- VDF improvements - VDF verification and generation speed has increased and dependence on flint2 has been removed. We wish to thank Dr. William Hart (@wbhart) for dual licensing parts of his contributions in FLINT and Antic for inclusion in the staicoin blockchain.
...
- Added color logging
- Now staicoin_harvester will periodically announce which plots it is currently farming and their k sizes.

### Changed

- Moved the ssh UI to use RPC.
- Changed the displayed process names for harvester, farmer, fullnode, timelords, and VDFs to to staicoin_full node, staicoin_harvester, etc. Fixed a bug that could cause inadvertent shutdown of other processes like an ongoing plotting session when new staicoin services were started.
...
- This is the first release of the staicoin testnet! Blockchain consensus, proof of time, and proof of space are included.
- More details on the release at [https://www.staicoin-network.net/developer/](https://www.staicoin-network.net/developer/)

[unreleased]: https://github.com/pinksheetscrypto/staicoin-blockchain/compare/1.0beta5...dev
[1.0beta5]: https://github.com/pinksheetscrypto/staicoin-blockchain/compare/1.0beta4...1.0beta5
[1.0beta4]: https://github.com/pinksheetscrypto/staicoin-blockchain/compare/1.0beta3...1.0beta4
[1.0beta3]: https://github.com/pinksheetscrypto/staicoin-blockchain/compare/1.0beta2...1.0beta3
[1.0beta2]: https://github.com/pinksheetscrypto/staicoin-blockchain/compare/1.0beta1...1.0beta2
[1.0beta1]: https://github.com/pinksheetscrypto/staicoin-blockchain/compare/alpha-1.5.1...1.0beta1
[alpha 1.5.1]: https://github.com/pinksheetscrypto/staicoin-blockchain/compare/alpha-1.5...alpha-1.5.1
[alpha 1.5]: https://github.com/pinksheetscrypto/staicoin-blockchain/compare/alpha-1.4.1...alpha-1.5
[alpha 1.4.1]: https://github.com/pinksheetscrypto/staicoin-blockchain/compare/alpha-1.4...alpha-1.4.1
[alpha 1.4]: https://github.com/pinksheetscrypto/staicoin-blockchain/compare/alpha-1.3...alpha-1.4
[alpha 1.3]: https://github.com/pinksheetscrypto/staicoin-blockchain/compare/alpha-1.2...alpha-1.3
[alpha 1.2]: https://github.com/pinksheetscrypto/staicoin-blockchain/compare/alpha-1.1.1...alpha-1.2
[alpha 1.1.1]: https://github.com/pinksheetscrypto/staicoin-blockchain/compare/alpha-1.1...alpha-1.1.1
[alpha 1.1]: https://github.com/pinksheetscrypto/staicoin-blockchain/compare/alpha-1.0...alpha-1.1
[alpha 1.0]: https://github.com/pinksheetscrypto/staicoin-blockchain/releases/tag/Alpha-1.0
